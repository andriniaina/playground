...
module TechnicalTests = 
    ...
    let [<Fact>] ValueAt() =
        [5.0 ; 2.0; 3.0]
            |> DenseVector.ofList
            |> NeuralNetworks.ValueAt 10.0 |> should equal (5.0 + 10.0*2.0 + 3.0*10.0*10.0)

    let [<Fact>] RegressionCoefficients() =
        let xdata = [ 1.0; 2.5; 3.0 ]
        let ydata = xdata |> List.map (fun x -> 5.0+2.0*x+3.0*x*x)
        let coeffs =
            NeuralNetworks.RegressionCoefficients 2 xdata ydata
            |> Seq.map round6
            |> List.ofSeq
    ...
    let [<Fact>] Derive() =
        ...
        let coeffs' =
            coeffs
            |> NeuralNetworks.Derive
        coeffs' |> Seq.map round6 |> List.ofSeq |> should equal [2.0; 6.0]
        NeuralNetworks.ValueAt 1.33 coeffs' |> round6 |> should equal (2.0+6.0*1.33)


    let [<Fact>] ``Regress``() =
        let xdata = [ 1.0; 2.5; 3.0 ]
        let ydata = [ 15.0; 25.0; 30.0 ]
        Math.Round(NeuralNetworks._Regress  2 (xdata) (ydata)    1.0  , 6) |> should equal 15.0
        Math.Round(NeuralNetworks._Regress  2 (xdata) (ydata)    2.0  , 6) |> should equal 20.833333
        Math.Round(NeuralNetworks._Regress  2 (xdata) (ydata)    2.5  , 6) |> should equal 25.0
        Math.Round(NeuralNetworks._Regress  2 (xdata) (ydata)    3.0  , 6) |> should equal 30.0
        Math.Round(NeuralNetworks._Regress  2 (xdata) (ydata)    4.0  , 6) |> should equal 42.5
        
    let [<Fact>] ``PredictTrend fonction croissante lineaire, +1 per min``() =
        let start = DateTime.Now
        let data = [ for i in 10..20 -> start + TimeSpan(0,i,0),  float i]
        Math.Round(NeuralNetworks.PredictTrend 30 data, 6) |> should equal 30.0
        Math.Round(NeuralNetworks.PredictTrend 60 data, 6) |> should equal 60.0
        
    let [<Fact>] ``Slope fonction croissante lineaire, +1 per min``() =
        let start = DateTime.Now
        let data = [ for i in 10..20 -> start + TimeSpan(0,i,0), float i]
        NeuralNetworks.Slope30perMin data |> round6 |> should equal 1.0
        NeuralNetworks.Slope60perMin data |> round6 |> should equal 1.0
