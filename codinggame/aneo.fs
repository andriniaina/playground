...
let trafficLightData = [for i in 0 .. nbLights - 1 do yield trafficLightDataParser (read string)]
...
let computePossibleTimeIntervals t = Seq.initInfinite (fun i -> i*2) |> Seq.map (fun i -> 
            (double i*t,double (i+1)*t)
    )
...
let intersectIntervals (intervalList1: (double*double) list) (intervalList2: (double*double) seq) = [
    for v1,v2 in intervalList1 do
        ...
        for w1,w2 in possibleIntersections do
            let borneGauche = if v1>w1 then v1 else w1 //Math.Max(v1,w1)
            let borneDroite = if v2<w2 then v2 else w2 //Math.Min(v2,w2)
...
| head::tail -> 
        let allIntersections =
            tail
            |> List.fold (fun state input -> intersectIntervals state input ) head
        ...
        let maxSpeedInKph =
            ...
            |> Seq.choose (fun (v1,v2) -> if v1>maxSpeedAllowedInKph then None else Some(Math.Min(maxSpeedAllowedInKph,v2)))
            |> Seq.head
            |> int
