...
eprintfn "maxSpeedAllowedInKph=%f;maxSpeedAllowedInMps=%f" maxSpeedAllowedInKph
    maxSpeedAllowedInMps
eprintfn "nbLights=%i" nbLights

let computePossibleTimeIntervals t =
    Seq.initInfinite (fun i -> i * 2)
    |> Seq.map (fun i -> (double i * t, double (i + 1) * t))
...
let intersectIntervals (intervalList1 : (double * double) list)
    (intervalList2 : (double * double) seq) =
    [ for v1, v2 in intervalList1 do
          let possibleIntersections =
              intervalList2
              |> Seq.takeWhile (fun (w1, w2) -> w1 > v1 || w2 > v1)
          for w1, w2 in possibleIntersections do
              ...
              if borneGauche <= borneDroite then
                  yield (borneGauche, borneDroite) ]

match speedIntervalsList with
| [] -> failwith "PAS BIEN"
| head :: tail ->
    let allIntersections = tail |> List.fold intersectIntervals head

    let allIntersectionsInKph =
        allIntersections
        |> List.map
               (fun (v1, v2) ->
               Math.Ceiling(mpsToKph v1), Math.Floor(mpsToKph v2))
