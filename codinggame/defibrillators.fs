...
let defibsReader = seq {
    for i in 0 .. N - 1 do
        let [|id;name;address;phonenum;slon;slat|] = Console.In.ReadLine().Split(';')
        yield (id,name,address,phonenum,asFloat slon,asFloat slat)
}
let defibs = List.ofSeq(defibsReader)

let computeDistance (longitudeA:float,latitudeA:float) (longitudeB:float,latitudeB:float) =
    let x = (longitudeB-longitudeA)*Math.Cos((latitudeA+latitudeB)/2.0)
    let y = latitudeB-latitudeA
    let d = Math.Sqrt(x*x+y*y)*6371.0
    d
let computeDistance2 (longitudeA,latitudeA) (id,name,address,phonenum,lon,lat)  =
    let d = computeDistance (longitudeA,latitudeA) (lon,lat)
    (name,d)

    
let distances = defibs |> List.map (fun defib -> computeDistance2 (LON,LAT) (defib))
