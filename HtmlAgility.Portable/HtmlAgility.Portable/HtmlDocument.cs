...
namespace HtmlAgilityPack
{
    ...
    public class HtmlDocument
    {
        ...
        private int _index;
        internal IDictionary<string, HtmlNode> _lastnodes = new Dictionary<string, HtmlNode>();
        private HtmlNode _lastparentnode;
        private int _line;
        private int _lineposition, _maxlineposition;
        internal IDictionary<string, HtmlNode> _nodesid;
        private ParseState _oldstate;
        private bool _onlyDetectEncoding;
        internal IDictionary<int, HtmlNode> _openednodes;
        ...
        /// </summary>
        public Encoding OptionDefaultStreamEncoding = Encoding.Unicode;
        ...
        */
        ...
        public void Load(TextReader reader)
        {
            ...
            if (OptionUseIdAttribute)
            {
                _nodesid = new Dictionary<string, HtmlNode>();
            }
            else
            {
                _nodesid = null;
        ...
        */
        ...
        private void CloseCurrentNode()
        {
            ...
            HtmlNode prev = _lastnodes.ContainsKey(_currentnode.Name) ? _lastnodes[_currentnode.Name] : null;
            if (prev == null)
            {
        ...
        {
            HtmlNode resetter = _lastnodes.ContainsKey(name) ? _lastnodes[name] : null;
            if (resetter == null)
                return null;
            if (resetter.Closed)
            {
        ...
        private void FixNestedTag(string name, string[] resetters)
        {
            ...
            prev = _lastnodes.ContainsKey(name) ? _lastnodes[name] : null;
        ...
        private void Parse()
        {
            ...
            _lastnodes = new Dictionary<string, HtmlNode>();
            _c = 0;
            _fullcomment = false;
            ...
            while (_index < _text.Length)
            {
                ...
                switch (_state)
                {
                    ...
                    case ParseState.PcData:
                        ...
                        {
                            if (_text.Substring(_index - 1, _currentnode._namelength + 2).ToLowerInvariant() == ("</" + _currentnode.Name).ToLowerInvariant())
                            {
        ...
        private bool PushNodeEnd(int index, bool close)
        {
            ...
            if ((close) || (!_currentnode._starttag))
            {
                if ((OptionStopperNodeName != null) && (_remainder == null) &&
                    (_currentnode.Name.ToLowerInvariant() == OptionStopperNodeName.ToLowerInvariant()))
                {
        ...
        private void ReadDocumentEncoding(HtmlNode node)
        {
            ...
            if (att == null)
                return;
            if (att.Value.ToLowerInvariant() != "content-type")
                return;
